...
@section('content')
    <div class="max-w-xl mx-auto bg-white rounded-2xl shadow-sm border border-slate-100 p-8 space-y-6">
        <h2 class="text-xl font-bold text-slate-900">Edit Experience Count</h2>

        <form action="{{ url('admin/experiences/' . $experience->id . '/update') }}" method="POST" class="space-y-4">
            @csrf
            <div>
                <label class="block text-xs font-semibold text-slate-600 uppercase mb-2">Count Number</label>
                <input type="number" name="count" value="{{ $experience->count ?? old('count') }}" required
