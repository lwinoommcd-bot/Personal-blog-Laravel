...
@section('content')
    <div class="max-w-5xl mx-auto bg-white mt-20 p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100">
        <h2 class="text-xl font-bold text-gray-900 mb-6">Add New Skill</h2>

        <form action="{{ url('admin/skills') }}" method="POST" class="space-y-5" novalidate>
            @csrf
            <!-- Skill Name Input -->
            <div>
                <label for="name" class="block text-sm font-medium text-gray-700 mb-1.5">Skill Name</label>
                <input type="text" name="name" value="{{ old('name') }}" id="name" placeholder="e.g. Laravel, Tailwind CSS"
                    required
            ...
            <div>
                ...
                <input type="number" value="{{ old('percent') }}" name="percent" id="percent" min="0" max="100"
